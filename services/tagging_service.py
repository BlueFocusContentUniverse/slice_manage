...
class TaggingService:
    ...
    async def initialize(self):
        ...
        if not self.session:
            ...
            # 直接使用KnowledgeBaseHandler已经初始化的会话头信息
            # 不再重复调用_login()
            if hasattr(self.kb_handler, 'session') and hasattr(self.kb_handler.session, 'headers'):
                self.session.headers.update(self.kb_handler.session.headers)
            else:
                self.logger.warning("KnowledgeBaseHandler的会话未初始化，可能需要先调用kb_handler._login()")
        ...
        try:
            ...
            for i in range(0, len(collections), batch_size):
                ...
                for collection in batch:
                    ...
                    # 获取该集合的问题分析结果
                    tag_name = await self.kb_handler.process_collections_questions([collection])
